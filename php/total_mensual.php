...
if ($accion == 1) {
    $resultado = $mysqli->query("SELECT id, nombre FROM ciudad WHERE estado = 1 ORDER BY nombre");
    $ciudades = [];
...
if ($accion == 2) {
    ...
    if (is_array($ciudades)) {
        foreach ($ciudades as $id => $valor) {
            ...
            $stmt = $mysqli->prepare("SELECT estado FROM ciudad WHERE id = ?");
            ...
            if ($estado != 1) {
                $ok = false;
                $errors[] = "Ciudad ID $id no está activa";
                continue;
            }
